...
// Le fichier est ecrit en little-endian pour accelerer les lectures/ecritures sur les plateformes Intel
//
...
{
    if (m_isLittleEndian) {
        readBytes((unsigned char *)&int4, 4);
    }
    else {
        ...
        int4 = ((bytes[3]*256 + bytes[2])*256 + bytes[1])*256 + bytes[0];
    }
}
////////////////////////////////////////////////////////////
...
{
    if (m_isLittleEndian) {
        writeBytes((unsigned char *)&int4, 4);
    }
    else {
        //un entier, c'est 4 octets sur le fichier lexique
        unsigned char bytes[4];
        ...
        bytes[0]=(int4)&0xFF;
        bytes[1]=(int4>>8)&0xFF;
        bytes[2]=(int4>>16)&0xFF;
        bytes[3]=(int4>>24)&0xFF;
        writeBytes(bytes, 4);
    }
}
////////////////////////////////////////////////////////////
