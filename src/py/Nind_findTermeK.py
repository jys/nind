...
usage   : %s <fichier lexiconindexK> <terme>
exemple : %s box/dumps/boxon/FRE.lexiconindexK prestation#maintenance#associé
...
def main():
    ...
    #terme = sys.argv[2].decode('latin1').strip()
    ...
    for mot in termesSimples:
        #2)trouve les clefs des termes simples
        clefA = NindLateconFile.clefA(mot)
        clefB = NindLateconFile.clefB(mot)
        index = clefB % nombreIndirection
        print '[%08X %08X]  %06X  %s'%(clefA, clefB, index, mot)
        ...
        if offsetDefinition != 0: 
            ...
            while inFile.tell() < finDonnees:
                ...
                fclefA = inFile.litNombre4()
                fclefB = inFile.litNombre4()
                cestLui = (fclefA == clefA) & (fclefB == clefB)
                identifiantS = inFile.litNombre3()
                nbreComposes = inFile.litNombreULat()
                mapTermes.append('[%08X %08X] %06d (%d)'%(fclefA, fclefB, identifiantS, nbreComposes))
