...
//
////////////////////////////////////////////////////////////
#include "NindLexicon/NindLexiconB.h"
    ...
    try {
        ...
        //while (!docsFile.eof()) {
            ...
            for (list<string>::const_iterator wordIt = wordsList.begin(); wordIt != wordsList.end(); wordIt++) {
                const unsigned int id = nindLexicon.addWord(*wordIt);
        ...
        start = clock();
        struct NindLexiconB::LexiconSizes lexiconSizes;
        ...
        for (list<pair<unsigned int, string> >::const_iterator wordIt = allWords.begin(); wordIt != allWords.end(); wordIt++) {
            const unsigned int id = nindLexicon.getId(wordIt->second);
        ...
        start = clock();
        for (list<pair<unsigned int, string> >::const_iterator wordIt = allWords.begin(); wordIt != allWords.end(); wordIt++) {
            const string word = nindLexicon.getWord(wordIt->first);
            if (word != wordIt->second) throw IntegrityException(wordIt->second);
        }
        end = clock();
        cpuTimeUsed = ((double) (end - start)) / CLOCKS_PER_SEC;
        cout<<"OK en "<<cpuTimeUsed<<" secondes"<<endl;
